
## Heartbeat 22:30 PST (2026-01-30)

### T721 COMPLETED ✅
Asset Correlation Heatmap Widget for /betting dashboard:
- Created `CorrelationHeatmapWidget.tsx` with 3x3 correlation matrix
- Visual heatmap showing BTC/ETH/Weather correlations
- Color-coded: red (high correlation) → green (diversified) → blue (hedging)
- Risk level badge, dynamic crypto limits, current prices
- Added `assetCorrelation` field to TradingStats interface
- Updated `push-stats-to-gist.py` with `load_asset_correlation()` function

### T722 COMPLETED ✅
Deployed correlation widget to onde.surf:
- Build via `npm run build:cf` - passed
- Deploy URL: https://3888e1b1.onde-surf.pages.dev
- Site working (307 redirect to login as expected)

### T723 COMPLETED ✅
Correlation history tracking:
- Modified `btc-eth-correlation.py` to log to `correlation-history.jsonl`
- `log_to_history()` appends snapshots with timestamp, correlation, prices
- `load_previous_correlation()` reads last value for comparison
- Added daily cron job at 12:00 UTC

### T724 COMPLETED ✅
Correlation change alerts:
- Added `check_and_alert_change()` function
- Alerts when correlation crosses 0.7→0.9 threshold (regime change)
- Alerts on any 15%+ change in correlation value
- Creates `kalshi-correlation-change.alert` for heartbeat pickup

---

## Heartbeat Progress (afternoon)

### Tasks Completed
- **T361**: Added PnL summary bar to /trading/history - shows total PnL, W/L/P breakdown, win rate, date range
- **T341**: Added keyboard navigation to trade history table - vim-style j/k navigation, pagination shortcuts, help modal

### Stats
- Autotrader: running (PID 7498)
- No alert files pending
- Git commits pushed successfully

---

## Heartbeat 14:38 PST

### T368 COMPLETED ✅
Edge Distribution Chart on /betting dashboard:
- Created EdgeDistributionChart component (6 buckets: 0-5%, 5-10%, 10-15%, 15-20%, 20-30%, 30%+)
- Color-coded bars by win rate (green ≥50%, yellow 40-50%, red <40%)
- Correlation indicator shows if model is well-calibrated
- Added calculate_edge_distribution() to push-stats-to-gist.py
- Updated gist with edge distribution data

### T444 COMPLETED ✅
Streak Context Indicator on /betting dashboard:
- Created StreakIndicator component with compact + full modes
- Compact badge in header showing current streak (🔥/❄️)
- Full card with best/worst streak comparison
- Recommendations based on streak patterns (tilt risk, hot streak, etc.)
- Intensity-based styling

### T445 COMPLETED ✅
API Latency Category Breakdown on /betting dashboard:
- Created ApiLatencyChart component with horizontal bars
- Shows Binance, CoinGecko, Coinbase, Other categories
- Color-coded by latency thresholds
- Health indicator badge with overall avg
- Interactive hover for p95/max details

### Tasks Added
- T443: Weather market performance widget
- T444: ✅ Completed
- T445: ✅ Completed

### Status
- Autotrader: running (PID 66542)
- No alert files
- All commits pushed to main

## 2026-01-31 14:54 PST - Heartbeat Work Session

### Tasks Completed
1. **T443 - Weather Market Performance Widget** ✅
   - Created WeatherPerformanceWidget.tsx component
   - Shows weather vs crypto performance comparison
   - City breakdown with PnL and win rates
   - NWS forecast accuracy tracking
   - "Coming Soon" state when no weather trades yet

2. **T447 - Deploy onde.surf** ✅
   - Successfully deployed to Cloudflare Pages
   - Deploy URL: https://51993bc9.onde-surf.pages.dev
   - Site working with auth redirect

### New Tasks Added
- T446: Health check JSON API for external monitoring
- T447: Deploy onde.surf (done)
- T448: PnL chart comparing weather vs crypto markets

### Technical Notes
- Fixed lint errors in EdgeDistributionChart, LatencyTrendChart
- Build + build:cf + wrangler deploy workflow confirmed working
- Autotrader running (PID 66542), no alerts


## 2026-01-31 15:30 - Heartbeat Work Session

### Tasks Completed
1. **T456**: Added auto-refresh toggle to /health page
   - Toggle switch (green=on, gray=off) with localStorage persistence
   - 30s auto-refresh when enabled
   - Loading spinner on refresh button
   - Last refresh timestamp in header and footer

2. **T458**: Deployed onde.la with auto-refresh toggle
   - Build passed, 16/17 tests pass (favicon.ico is known false positive)
   - Live at: https://onde.la/health

### Tasks Added
- T459: Keyboard shortcut for manual refresh (R key)
- T460: Browser notifications on status change

### System Status
- Autotrader: Running (PID 66542)
- No alerts pending
- onde.la: 200 OK
- onde.surf: Working


---

## 2026-01-31 16:05 - SE-Bot Embeddings Setup

### Heartbeat Work
Continued SE-Bot knowledge base development (T472):

1. **Created `build_embeddings.py`**: ChromaDB indexer for KB markdown files
   - Section-based chunking for better retrieval
   - Uses sentence-transformers (all-MiniLM-L6-v2) - local, no API key
   - Indexes all markdown files from knowledge-base/

2. **Created `kb_search.py`**: Search module for SE-Bot integration
   - `search(query)` - basic semantic search
   - `get_context_for_topic(topic)` - formatted context for Claude prompts
   - Singleton pattern for ChromaDB connection

3. **Updated README.md** with setup instructions

4. **Added 3 new tasks**:
   - T478: Test embeddings search quality
   - T479: Create meeting simulator for testing
   - T480: Trading portfolio-wide position concentration limits

### Issue Encountered
Python 3.14 + pydantic v2 compatibility issue with chromadb. Upgrading chromadb to latest version. May need to wait for upstream fix or use Python 3.12.

### Commits
- `8f5f8edad` - feat(se-bot): add ChromaDB embeddings setup for knowledge base

## 2026-01-31 12:30 - Portfolio Concentration Limits (T480)

### Task Completed
Implemented portfolio-wide position concentration limits for the autotrader.

### Changes Made
1. **Config constants** (lines ~215):
   - `CONCENTRATION_MAX_ASSET_CLASS_PCT = 0.50` (50% max per asset class)
   - `CONCENTRATION_MAX_CORRELATED_PCT = 0.30` (30% max for correlated groups)
   - `CONCENTRATION_WARN_PCT = 0.40` (40% warning threshold)
   - Alert file: `kalshi-concentration.alert`

2. **Functions added**:
   - `classify_asset_class(ticker)`: Classifies position as btc/eth/weather/other
   - `get_correlated_group(ticker)`: Groups correlated assets (crypto = BTC+ETH)
   - `calculate_portfolio_concentration()`: Calculates current concentration metrics
   - `check_concentration_limits()`: Validates if new trade is allowed
   - `write_concentration_alert()`: Writes warning alert file
   - `print_concentration_summary()`: Visual concentration display

3. **Integration**:
   - Concentration summary displayed each cycle when positions exist
   - Trades blocked if they would exceed limits
   - Skipped trades logged to `kalshi-skips.jsonl` with concentration reason
   - Trade data now includes concentration_asset_pct, concentration_corr_group_pct

### New Tasks Added
- T481: Rebalancing suggestions when concentration high
- T482: Historical concentration tracking dashboard widget
- T483: BTC/ETH correlation tracking for smarter diversification

## 2026-01-31 ~08:15 PST - Heartbeat: Completed T488 Ambient Soundscapes

### Task Completed
**T488**: Moonlight House ambient soundscapes per room
- Created `apps/moonlight-house/src/components/AmbientSoundscapes.tsx`
- Procedural Web Audio API-based soundscapes (no audio files needed!)
- All 8 rooms have unique ambiance:
  - Bedroom: Music box, clock tick
  - Kitchen: Sizzling, bubbling, fridge hum
  - Garden: Birds (day) ↔ Crickets (night) - time-aware!
  - Living: TV murmur, clock
  - Bathroom: Water drips, steam (reverb)
  - Garage: Tool clanks, engine idle
  - Shop: Sparkle chimes, piano notes
  - Supermarket: Checkout beeps, cart squeaks
- Features: LFO modulation, convolver reverb, randomized accents

### New Tasks Added
- T665: Deploy Moonlight House with soundscapes to onde.la (P2)
- T666: Add separate volume slider for ambient (P3)
- T667: Weather-based ambient variations (P4)

### System Status
- Autotrader: Running (PID 66542)
- No alert files present
- Git: Committed and pushed

## 2026-01-31 18:57 PST - T668 Reader App Prototype COMPLETED

### Task Completed
- **[T668]** Reader App: Prototype Web-Based Book Reader
- Built full MVP in `apps/reader-app/`

### Features Implemented
1. **Library View**
   - Book grid with covers
   - Progress tracking
   - "Continue Reading" section
   - Upload EPUB button (IndexedDB storage ready)

2. **Reader View**
   - Themes: Light, Dark, Sepia
   - Font size: 14-28px slider
   - Font family: serif/sans toggle
   - Line height: 1.4-2.2 slider
   - Margins: small/medium/large

3. **Navigation**
   - Swipe left/right for pages
   - Arrow keys, Space, Escape shortcuts
   - Table of Contents slide-out
   - Progress bar at top

4. **State Management**
   - Zustand store
   - localStorage persistence
   - Per-book progress tracking

### Tech Stack
- Next.js 15 + Tailwind + Zustand
- PWA manifest ready
- Run: `cd apps/reader-app && npm run dev`

## 2026-01-31 19:10 PST - Heartbeat Work: T693 Reader Navigation

### Completed
- **T693**: Added Reader App link to onde.la navigation
  - Added to main nav (desktop + mobile): "Read" / "Leggi"
  - Added to footer Explore section
  - Deployed: https://48afb914.onde-portal.pages.dev
  - Both onde.la and onde.la/reader/ verified 200 OK

### Status
- Autotrader running (PID 66542)
- No alert files
- Picking next task...

## 2026-01-31 19:20 PST - T690 Reader App EPUB Support

### Completed
- **T690**: Full EPUB Support for Reader App
  - Created EpubReader component (src/components/EpubReader.tsx)
  - Uses epub.js for rendering real EPUB files
  - Features:
    - Metadata extraction (title, author, cover)
    - Chapter navigation via Table of Contents
    - Progress tracking with epub.js locations
    - Theme support applied to EPUB content
    - Touch swipe + keyboard navigation
  - Created shared epubStorage module (src/lib/epubStorage.ts)
  - Added TypeScript types for epub.js
  - Sample books can now load from Project Gutenberg URLs
  - Build successful

### Files Created/Modified
- `apps/reader-app/src/components/EpubReader.tsx` (new)
- `apps/reader-app/src/lib/epubStorage.ts` (new)
- `apps/reader-app/src/types/epub.d.ts` (new)
- `apps/reader-app/src/app/page.tsx` (updated)
- `apps/reader-app/src/components/Library.tsx` (updated)
## 2026-01-29 19:21 - Heartbeat Tasks

### Completed
- **[T691]** Reader App: Bookmarks & Highlighting
  - HighlightMenu component (4 colors, notes support)
  - AnnotationsPanel (tabs, search, filter, export)
  - Bookmark toggle in header
  - epub.js integration for rendering highlights
- **[T697]** Deployed to onde.la/reader
  - Build with STATIC_EXPORT=1
  - Deployed via Cloudflare Pages
  - Verified: https://onde.la/reader/ live

### Added Tasks
- T697 (done), T698 (sync), T699 (dictionary)


## 2026-01-31 19:33 PST - Heartbeat: Dictionary Feature Complete

### Tasks Completed
- **T699**: Reader App dictionary lookup on word select
  - DictionaryPopup component using dictionaryapi.dev (free, no key)
  - Single word detection in HighlightMenu
  - Audio pronunciation playback
  - "Add to Vocabulary" → VocabularyWord store
  - "Save as Highlight" with definition note
  - VocabularyPanel for managing/exporting saved words
  - 📚 button in reader header

- **T700**: Deployed dictionary feature to onde.la
  - https://f8d29668.onde-portal.pages.dev
  - https://onde.la/reader/ verified 200 OK

### Technical Details
- New components: DictionaryPopup.tsx, VocabularyPanel.tsx
- Store extended with VocabularyWord type and actions
- Export vocabulary to Markdown feature
- Build OK, all features working


## 2026-01-31 04:23 PST - Heartbeat: VR Reader Deploy

### Completato
- **T704 DONE**: Deployed VR Reader to onde.la/reader-vr/
  - Configured next.config.ts for static export (basePath, assetPrefix, transpilePackages)
  - Deployed via wrangler: https://3dc1ab5c.onde-portal.pages.dev
  - Verified: https://onde.la/reader-vr/ returns 200 OK
- Added VR mode link (🥽) to Reader App library header
  - Purple button linking to /reader-vr/
  - Redeployed Reader App: https://65575568.onde-portal.pages.dev

### Stato Sistema
- Autotrader: Running (PID 66542)
- Alert files: None
- Sites: onde.la ✅, onde.surf ✅

### Next Task
- T703: EPUB text extraction for VR
- T696: Surfboard upgrade to Next.js 15

## 2026-01-31 04:27 PST - Heartbeat Continued: EPUB Support for VR

### Completato
- **T703 DONE**: EPUB text extraction for VR rendering
  - Created `lib/epubParser.ts` - plain text extraction from EPUB
  - Created `store/bookStore.ts` - Zustand store with persistence
  - Added `BookSelector.tsx` - modal for book selection
  - Updated FloatingBook + VRScene for dynamic content
  - Added epub.d.ts types
  - Demo books: Pride & Prejudice, Moby Dick, Frankenstein
  - Smart pagination (~800 chars/page)
  - Deployed: https://b938a9a1.onde-portal.pages.dev

### Sessione Summary
Completati 2 task P2:
1. T704: VR Reader deploy
2. T703: EPUB support integration

VR Reader ora supporta:
- 3 libri demo da Project Gutenberg
- Parsing EPUB → testo plain per VR
- Progress tracking con localStorage
- Chapter navigation

## 2026-01-31 17:30 - Heartbeat: VR Reader Environment Enhancement

### Task Completed
- **[T702] Reader App VR: Cozy library environment** - DONE ✅
  - Enhanced ReadingEnvironment.tsx with 25+ new 3D elements
  - Rich wooden walls with wainscoting and crown molding
  - Fireplace with animated flames
  - Grandfather clock with pendulum
  - Window with curtains and dynamic lighting
  - Time-of-day system (morning/afternoon/evening/night)

### Technical Details
- Used R3F (React-Three-Fiber) with drei helpers
- Lighting presets for each time of day
- Animated elements: flames, pendulum, globe rotation, floating candles
- Fixed TypeScript issues with cylinderGeometry rotation

### Git
- Committed: 2e6d0f721
- Pushed to main

### Task Completed (continued)
- **[T708] Reader App VR: Deploy enhanced version** - DONE ✅
  - Deployed: https://01d44bf8.onde-portal.pages.dev
  - Verified: https://onde.la/reader-vr/ returns 200 OK
  - All enhanced features now live

## 2026-01-31 21:17 - Reader VR Ambient Soundscapes (Heartbeat)

### Completed
- **T707**: Added procedural ambient soundscapes to VR Reader
  - Created `apps/reader-vr/components/AmbientSoundscapes.tsx`
  - Library-specific sounds: fireplace, clock, page turns
  - Time-of-day variations (birds morning, crickets night)
  - Mute + volume controls in UI
  - Deployed: https://onde.la/reader-vr/

### Technical
- Based on Moonlight House T488 pattern
- All Web Audio API procedural (no files)
- Integrated page turn sound with handleNextPage/handlePrevPage wrappers

### Next
- Continue with TODO tasks from TASKS.md

## 2026-01-31 21:30 - Heartbeat: TTS Audiobook Mode Implemented

### Task Completed
**[T712] Reader App: Add text-to-speech audiobook mode** - DONE

### Implementation Details
- Created `TextToSpeech.tsx` component with Web Speech API
- Voice selection grouped by language from system voices  
- Speed (0.5x-2x), pitch (0.5-2.0), volume (0-100%) controls
- Play/Pause/Stop controls with skip forward/backward by sentence
- Progress bar showing current position in text
- Current sentence display with sentence counter
- Smart sentence splitting on punctuation (.!?)
- Auto-advance to next sentence when finished
- 🎧 button added to reader header
- Text extraction from current epub page via 'rendered' event

### Deployed
- https://onde.la/reader/ - 200 OK ✅

### New Tasks Added
- [T713] Reader VR: Integrate TTS with spatial audio (P4, deprioritized)
- [T714] Reader App: TTS keyboard shortcuts in reader view (P3)
- [T715] Reader App: Auto-page-turn during TTS playback (P3)

### Git
Commit: a33e2cf07 - feat(reader-app): add text-to-speech audiobook mode [T712]

## 2026-01-31 21:39 PST - Heartbeat: Reader App TTS Improvements

### Tasks Completed
1. **T714** - TTS keyboard shortcuts in reader view
   - Space: Play/Pause
   - Escape: Close panel
   - Left/Right arrows: Previous/Next sentence
   - Up/Down arrows: Speed adjust (±0.1x)
   - M: Mute/unmute toggle
   - Updated shortcuts hint UI

2. **T715** - Auto-page-turn during TTS playback
   - Auto-detects when all sentences on page are spoken
   - Calls goToNext() to flip page automatically
   - "Loading next page..." indicator during transition
   - Toggle switch to enable/disable
   - Uses ref to track playing state across transitions

3. **T716** - Deploy TTS features to onde.la/reader
   - Deployed: https://0146b182.onde-portal.pages.dev
   - Verified: https://onde.la/reader/ returns 200 OK

### New Tasks Added
- T717: Save TTS settings to localStorage (P3)
- T718: Add TTS speed presets slow/normal/fast (P4)

### Technical Notes
- Fixed circular dependency issue in TextToSpeech.tsx by using refs
- Used isPlayingRef to track state across async callbacks
- shouldAutoContinueRef for page transition continuity

## 2026-01-31 06:55 PST - Heartbeat: SE-Bot Voice Output Complete

### Work Done
- **[T475] SE-Bot Voice Output** - DONE ✅
  - Created `apps/se-bot/voice_output.py` (732 lines)
  - ElevenLabs TTS API integration with full feature set
  - 9 popular voices pre-configured + full API catalog access
  - Voice presets: professional, expressive, calm, energetic
  - Voice cloning support (paid plans)
  - Audio caching to reduce API costs
  - BlackHole virtual mic output for meeting injection
  - Streaming mode for lower latency
  - macOS TTS fallback when no API key
  - Updated se_bot_main.py integration
  - Updated requirements.txt

### New Tasks Added
- [T734] SE-Bot: Add voice output to overlay suggestion cards (P2)
- [T735] SE-Bot: Add competitor detection and battle card mode (P2)
- [T736] Voice Output: Add speaking queue for multiple suggestions (P3)

### System Status
- Autotrader running: PID 66542 ✅
- No alert files ✅
- Git synced ✅

## 2026-01-31 07:15 PST - Heartbeat Complete: 2 Tasks Done

### Completed This Heartbeat
1. **[T475] SE-Bot Voice Output** - DONE ✅
   - ElevenLabs TTS API integration
   - 9 popular voices + full API catalog
   - Voice cloning support
   - BlackHole virtual mic output
   - macOS TTS fallback

2. **[T734] Voice Output in Overlay** - DONE ✅
   - 🔊 speaker button on each suggestion card
   - Click or Shift+1/2/3 to speak
   - Visual feedback while speaking
   - Background thread for non-blocking playback

### New Tasks Added
- [T734] SE-Bot voice in overlay (P2) - DONE
- [T735] Competitor detection + battle card mode (P2)
- [T736] Speaking queue for voice output (P3)

### System Status
- Autotrader: PID 66542 ✅
- Alert files: None ✅
- Git: Synced ✅

## 2026-01-31 ~16:15 - Heartbeat Work Session

### Tasks Completed
1. **T737** - Win rate trend sparkline added to /betting page
   - Created `WinRateSparkline.tsx` component
   - 7-day trend visualization inline with win rate stat
   - Color-coded by trend direction (green/red)
   - Build passes, committed & pushed

2. **T732** - Video avatar provider comparison doc for SE-Bot
   - Created `apps/se-bot/docs/VIDEO-AVATAR-COMPARISON.md`
   - Compared D-ID, HeyGen, Synthesia
   - Recommendation: D-ID for MVP (best latency, free tier)
   - Includes integration architecture and cost estimates
   - Committed & pushed

### Status
- Autotrader running (PID 66542)
- No alert files
- Git synced with main

## 2026-01-31 19:55 - Heartbeat: T735 Countdown Timer

### Task Completed
- **T735**: Dashboard countdown timer for auto-refresh on /health page
- Implemented circular SVG progress indicator
- 30s countdown with pause-on-hover feature
- Respects auto-refresh toggle state

### New Tasks Added
- **T739**: Deploy countdown to onde.la (P2)
- **T740**: Port countdown to /betting page (P3)
- **T741**: Add alert sound on status change (P4)

### Notes
- Build passes, committed and pushed
- Autotrader running (PID 66542)
- No alert files present

### T739 Deployed
- Deployed countdown timer to onde.la/health
- Deploy URL: https://e422ade3.onde-portal.pages.dev
- Verified: https://onde.la/health/ returns 200 OK

## 2026-01-31 ~21:15 - Heartbeat: CI/CD & Dashboard Improvements

### Tasks Completed
1. **T726** - CI/CD Pipeline con Feedback Loop Auto-Miglioramento (P1)
   - Created `.github/workflows/ci-cd.yml` with multi-stage pipeline
   - Lint → Test → Build → Deploy → Verify stages
   - Auto-deploy to Cloudflare on main branch
   - Nightly schedule (4 AM UTC) for auto-improvement cycles
   - Path-based triggers for efficiency
   - Created `scripts/ci-auto-improve.py` for failure analysis

2. **T737** - Win Rate Sparkline (discovered already implemented)
   - Verified `WinRateSparkline.tsx` already exists and integrated
   - Marked as DONE

3. **T742** - Keyboard hint badge for /betting refresh button (P4)
   - Added 'R' kbd badge to refresh button
   - Desktop only (hidden sm:inline-block)

4. **T745** - CI/CD Telegram notifications for failures (P3)
   - Added Telegram notification to auto-improve job
   - Notifies on any pipeline failure with details

5. **T670** - Marked as duplicate of T668 (Reader App already done)

### New Tasks Added
- **T745**: CI/CD Telegram notifications (now DONE)
- **T746**: Deploy preview URLs for PRs (P3)
- **T747**: Test coverage tracking (P4)

### Status
- Autotrader: PID 66542 ✅
- No alert files ✅
- Git: 4 commits pushed to main

## 2026-01-31 00:50 PST - Heartbeat Work Session

### Tasks Completed
- **[T743]** Dashboard: Show position age on open positions
  - Added position expiry indicator to /betting page
  - Parses KXBTCD/KXETHD ticker format (DDMMMYYHH) for expiration
  - Shows Clock icon + time remaining next to each position
  - Color coding: green (>24h), yellow (4-24h), red (<4h with pulse)
  
- **[T748]** Deploy to production
  - Deployed to https://b308a17a.onde-surf.pages.dev
  - Verified auth redirect working

### New Tasks Added
- T749: P&L percentage display
- T750: Position entry timestamp API
- T751: Daily volume indicator

### System Status
- Autotrader: Running (PID 66542)
- No alert files pending
