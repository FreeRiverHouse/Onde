...
// Sample text pages (Pride & Prejudice)
const BOOK_PAGES = [
  `PRIDE AND PREJUDICE
by Jane Austen
...
"In such cases, a mother has not often much beauty to think of."
...
export function FloatingBook({ 
  ...
  const pageIndex = currentPage % BOOK_PAGES.length;
    ...
    <group ref={groupRef} position={[0, 1.4, -distance]}>
      ...
      <group position={[-0.55, -0.8, 0.03]}>
        <mesh onClick={onPrevPage}>
          ...
          <meshBasicMaterial color="#333333" transparent opacity={pageIndex > 0 ? 0.8 : 0.3} />
        </mesh>
        <Text
          position={[0, 0, 0.01]}
          fontSize={0.04}
