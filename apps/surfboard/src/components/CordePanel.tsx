    ...
    <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-2">
          {status.authors.map(author => (
            <button
              key={author.id}
              onClick={() => setFeedbackTo(author.id)}
      ...
      <div className="border-t border-white/10 pt-4">
        <div className="text-sm text-white/60 mb-2">
          Feedback {feedbackTo ? `a ${status.authors.find(a => a.id === feedbackTo)?.name}` : ''}
        </div>
        <div className="flex gap-2">
          <input
            ...
            disabled={!feedbackTo}
            className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-cyan-500/50 disabled:opacity-50"
          />
          <button
            onClick={sendFeedback}
            disabled={!feedbackTo || !feedbackText.trim()}
            className="px-4 py-2 rounded-lg bg-cyan-500/20 text-cyan-400 text-sm hover:bg-cyan-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Invia
          </button>
        </div>
        {feedbackSent && (
          <div className="text-xs text-emerald-400 mt-2">Feedback inviato!</div>
        )}
      </div>
    </div>
  )
