...
// Top categories for quick-filter pills (most popular)
...
function CatalogSkeleton() {
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 animate-pulse">
      <div className="h-20 bg-stone-200 dark:bg-stone-700 rounded-2xl mb-8" />
      <div className="h-10 w-48 bg-stone-200 dark:bg-stone-700 rounded mb-4" />
      <div className="h-6 w-32 bg-stone-200 dark:bg-stone-700 rounded mb-8" />
      <div className="h-12 bg-stone-200 dark:bg-stone-700 rounded-lg mb-6" />
      <div className="flex gap-2 mb-4">
        {[1,2,3,4,5].map(i => (
          <div key={i} className="h-8 w-20 bg-stone-200 dark:bg-stone-700 rounded-full" />
        ))}
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {[1,2,3,4,5,6,7,8].map(i => (
          <div key={i} className="aspect-[3/4] bg-stone-200 dark:bg-stone-700 rounded-lg" />
        ))}
      </div>
    </div>
  )
...
// Main catalog content (uses useSearchParams)
function CatalogContent() {
  ...
  const [selectedLang, setSelectedLang] = useState<string | null>(searchParams.get('lang'))
  const [sortBy, setSortBy] = useState<SortOption>((searchParams.get('sort') as SortOption) || 'title')
  ...
  // Load sort preference from localStorage (only if not in URL)
  useEffect(() => {
  ...
  // Save sort preference and sync to URL
  const handleSortChange = (newSort: SortOption) => {
    setSortBy(newSort)
    localStorage.setItem('catalog-sort', newSort)
    updateURL({ sort: newSort === 'title' ? null : newSort })
  }
  ...
  // Handle category change with URL sync
  const handleCategoryChange = (category: string | null) => {
    setSelectedCategory(category)
    updateURL({ category })
  }

  // Handle language change with URL sync
    ...
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full px-4 py-3 rounded-lg border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-800 focus:outline-none focus:ring-2 focus:ring-onde-gold focus:border-transparent"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-sm transition-all ${
            !selectedCategory
              ? 'bg-onde-gold text-white shadow-md'
        ...
        {topCategories.map(cat => (
          ...
          >
            {cat}
            <span className="ml-1 opacity-60">({categoryMap[cat]})</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select
          ...
          className="px-4 py-2 rounded-lg border border-stone-300 dark:border-stone-600 bg-white dark:bg-stone-800 focus:outline-none focus:ring-2 focus:ring-onde-gold"
        >
          <option value="">{t.catalog.filters.allCategories}</option>
          {categories.map(cat => (
            <option key={cat} value={cat}>{cat} ({categoryMap[cat]})</option>
          ))}
        </select>
        ...
        {hasActiveFilters && (
          <button
            ...
            className="px-4 py-2 rounded-lg bg-stone-200 dark:bg-stone-700 hover:bg-stone-300 dark:hover:bg-stone-600 transition-colors"
          >
      ...
      {selectedCategory && (
        <div className="mb-4 flex items-center gap-2">
          <span className="text-sm opacity-60">Filtering:</span>
          <span className="px-2 py-1 bg-onde-gold/20 text-onde-gold rounded text-sm font-medium">
            {selectedCategory}
          </span>
          <button
            onClick={() => handleCategoryChange(null)}
          ...
          {filteredBooks.map(book => {
            ...
            return (
              <Link href={'/libro/' + book.id} key={book.id} className="book-card group">
                <div className="aspect-[3/4] bg-gradient-to-br from-stone-100 to-stone-200 dark:from-stone-700 dark:to-stone-800 rounded-lg mb-3 flex items-center justify-center">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {book.lang && book.lang !== 'en' && (
                    <span className="text-xs bg-stone-200 dark:bg-stone-700 px-2 py-1 rounded uppercase">{book.lang}</span>
                  )}
                </div>
              </Link>
            )
