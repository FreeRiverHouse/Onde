...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  if (nelements_nbytes := { 2: (32, 18), 3: (32, 20), 8: (32, 34), 12: (256, 144), 14: (256, 210), 39: (32, 17) }.get(ggml_type)) is not None:
    ...
    if ggml_type == 2: return ((q_to_uint8(blocks[:,2:], 4).bitcast(dtypes.int8) - 8) * blocks[:,:2].bitcast(dtypes.float16).cast(dtypes.float32)).cast(dtypes.float16)
    if ggml_type == 3:
      d, m = (blocks[:,s:s+2].bitcast(dtypes.float16).cast(dtypes.float32) for s in [ 0, 2 ])
      return (q_to_uint8(blocks[:,4:], 4).bitcast(dtypes.int8) * d + m).cast(dtypes.float16)
    if ggml_type == 8: return (blocks[:,:2].bitcast(dtypes.float16).cast(dtypes.float32) * blocks[:,2:].bitcast(dtypes.int8)).cast(dtypes.float16)
    ...
    if ggml_type == 14:
      ...
      return (d * (xl.bitwise_or(xh).bitcast(dtypes.int8) - 32).flatten(-2) * scales).cast(dtypes.float16)
    if ggml_type == 39:
