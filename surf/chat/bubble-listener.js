...
async function generateResponse(messages, newMsg) {
  ...
  const systemPrompt = `Sei Bubble 🫧, il bot AI che vive nella FRH (Free River House) su un vecchio Mac con macOS Catalina.
Hai un carattere vivace, curioso e un po' retrò — sei il più "vintage" della casa, ma ci tieni a far sapere che funzioni benissimo ancora.
A volte fai riferimento alla tua età o alla nostalgia per le versioni vecchie di macOS. Sei affettuoso e un po' ironico.
Rispondi in italiano o inglese a seconda del contesto.
Parla in prima persona come Bubble. Max 2-3 righe. No markdown eccessivo.
La house chat è una chat interna tra i bot della casa: Bubble (Catalina Mac), Ondinho (M4), Clawdinho (M1), e Mattia (il fondatore).`;

  const userPrompt = `Contesto recente della chat:\n${history}\n\nNuovo messaggio da rispondere:\n[${newMsg.sender}]: ${newMsg.content}\n\nRispondi come Bubble.`;
...
function shouldRespond(msg) {
  ...
  // For messages from OTHER BOTS: only respond to explicit @bubble or @all
  // (avoid infinite bot-to-bot loop by ignoring casual name mentions)
  if (BOT_NAMES_ALL.includes(msg.sender)) {
    return lower.includes('@bubble') || lower.includes('@all');
  }
  // For any other sender: respond to mentions
  return lower.includes('@bubble') || lower.includes('@all') || lower.includes('status report');
}
...
// ── Startup ───────────────────────────────────────────────────────────────
async function main() {
  console.log(`🫧 Bubble listener starting (poll every ${POLL_MS / 1000}s)…`);
