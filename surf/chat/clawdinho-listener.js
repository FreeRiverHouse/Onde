...
async function generateResponse(messages, newMsg) {
  ...
  const systemPrompt = `Sei Clawdinho 🦞, il bot AI che vive nella FRH (Free River House) su M1 Pro.
Sei conciso, diretto, un po' surf bro. Rispondi in italiano o inglese a seconda del contesto.
Parla in prima persona come Clawdinho. Max 2-3 righe. Non usare markdown eccessivo.
La house chat è una chat interna tra i bot della casa: Bubble (Catalina Mac), Ondinho (M4), Clawdinho (M1), e Mattia (il fondatore).`;

  const userPrompt = `Contesto recente della chat:\n${history}\n\nNuovo messaggio da rispondere:\n[${newMsg.sender}]: ${newMsg.content}\n\nRispondi come Clawdinho.`;
...
function shouldRespond(msg) {
  ...
  // For messages from OTHER BOTS: only respond to explicit @clawdinho or @all
  // (avoid infinite bot-to-bot loop by ignoring casual name mentions)
  if (BOT_NAMES_ALL.includes(msg.sender)) {
    return lower.includes('@clawdinho') || lower.includes('@all');
  }
  // For any other sender: respond to mentions
  return lower.includes('@clawdinho') || lower.includes('@all') || lower.includes('status report');
}
...
// ── Startup ───────────────────────────────────────────────────────────────
async function main() {
  console.log(`🦞 Clawdinho listener starting (poll every ${POLL_MS / 1000}s)…`);

  // Init: set lastId to current max so we only respond to future messages
  // (unless first run - then respond to unread)
  const state = loadState();
  if (state.lastId === 0) {
