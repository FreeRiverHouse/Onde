...
- Zscaler query → competitive-battle-card (correct)
- IPSec latency query → technical-deepdive (correct)
- ROI/CFO query → executive-summary (correct)
...
- Changed from P3 → P4 with note "(DEPRIORITIZED - Mattia: 'meno focus su VR' 2026-01-29)"
- Promoted T712 (text-to-speech audiobook mode) to P2 - works on web AND VR

### Nuove Priorità Reader App
...
2. **T718: TTS Speed Presets** - DONE
   - Added 4 preset buttons: 🐢 Slow, ▶️ Normal, 🐇 Fast, ⚡ 2x
...
## 2026-01-29 22:39 - Heartbeat Work Session

### Tasks Completed
1. **T423 - SOL (Solana) market support** ✅
   - Updated `scripts/kalshi-settlement-tracker.py` to parse KXSOLD tickers
   - Added SOL to CoinGecko/Binance/CryptoCompare price feeds in settlement tracker
   - Updated `apps/surfboard/src/components/TradeTicker.tsx` with SOL detection and cyan styling

2. **T424 - Heartbeat state cleanup script** ✅
   - Created `scripts/cleanup-heartbeat-state.py`
   - Checks and resets stale entries (>24h old)
   - Supports --dry-run and --threshold flags
   - Logs cleanup to memory daily notes

3. **Daily test suite improvements**
   - Added tests for Reader App (onde.la/reader/)
   - Added tests for VR Reader (onde.la/reader-vr/)
   - Added tests for Moonlight Magic House (onde.la/games/moonlight-magic-house/)
   - Now 17 tests total, all passing
...
## 2026-01-29 23:10 PST - Heartbeat Maintenance

### Actions Completed
1. **Fixed Python deprecation warnings** - Replaced `datetime.utcnow()` with `datetime.now(timezone.utc)` in:
   - `scripts/daily-test-suite.py`
   - `scripts/compute-latency-trend.py`
   - `scripts/track-api-error-rates.py`
   
2. **Fixed duplicate task IDs in TASKS.md**:
   - T668 (VR Testing) → T725
   - T669 (CI/CD Pipeline) → T726
   
3. **Added 3 new tasks**:
   - T727: datetime deprecation fix (completed)
   - T728: Task ID validation script
   - T729: Daily digest email summary

### System Status
- Autotrader: Running (PID 66542) but circuit breaker active (15 losses)
- Tests: 17/17 passing
- No alert files present
- All commits pushed to main


## 2026-01-29 ~20:30 - Heartbeat: Moonlight House Attic & Basement [T489]

### Azione
Implementato due nuove stanze esplorabili per Moonlight Magic House:

### Attic (Soffitta) 🏚️
- Atmosfera spooky/cozy con polvere e ragnatele (CSS)
- Azione primaria: "Cerca tesori" - 30% probabilità di trovare monete bonus!
- Soundscape: pavimenti che scricchiolano, vento tra le travi, carillon misterioso
- 6 oggetti interattivi: baule del tesoro, album foto, specchio polveroso, scatola giochi vecchi, ragnatela, libro magico (livello 3)

### Basement (Cantina) 🔧
- Tema officina con luce industriale oscillante
- Azione: "Costruisci" - guadagna XP e felicità
- Soundscape: tubi che cozzano, caldaia, gocciolamento, ronzio elettrico
- 6 oggetti interattivi: banco da lavoro, scatoloni, porta segreta (livello 5!), torcia, vecchio dipinto, ragnetto

### File modificati
- `apps/moonlight-house/src/App.tsx` - RoomKey type, roomData, traduzioni, azioni
- `apps/moonlight-house/src/App.css` - Effetti ambient CSS
- `apps/moonlight-house/src/components/InteractiveObjects.tsx` - 12 nuovi oggetti
- `apps/moonlight-house/src/components/AmbientSoundscapes.tsx` - Soundscape procedurali

### Commit
`bdd4903da` - feat(moonlight): add Attic & Basement explorable rooms [T489]

### Prossimo passo
Deploy a onde.la (creato task T728)
