#!/usr/bin/env python3
"""
🚀 TRANSLATION PIPELINE MLX - BOMPROOF v12.0 🚀
...
NOVITÀ v12:
  - Resume Capability: Riprende dal checkpoint_translated.txt in caso di crash
  - Incremental Save: Salva ogni paragrafo appena tradotto
...
def run_pipeline(input_file: str, output_dir: str):
    print("""
╔═══════════════════════════════════════════════════════════════════╗
║  🚀 TRANSLATION PIPELINE MLX v12.0 - QWEN3-32B OTTIMIZZATO        ║
║  📍 Resume On Crash & Incremental Saving enabled                  ║
╚═══════════════════════════════════════════════════════════════════╝
""")
    ...
    # Resume logic
    checkpoint_file = output_path / "checkpoint_translated.txt"
    start_index = 0
    translated = []
    failed = 0
    
    if checkpoint_file.exists():
        translated = checkpoint_file.read_text(encoding='utf-8').split("\n\n--------------------\n\n")
        # Remove empty string at end if present
        if translated and not translated[-1].strip():
            translated.pop()
        start_index = len(translated)
        if start_index > 0:
            log(f"🔄 Checkpoint trovato! Riprendo dal paragrafo {start_index+1}")
