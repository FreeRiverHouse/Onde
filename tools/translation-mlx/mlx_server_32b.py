...
# Config
PORT = 8766
LOCK_FILE = Path(__file__).parent / "server_32b.lock"
PID_FILE = Path(__file__).parent / "server_32b.pid"
STATUS_FILE = Path(__file__).parent / "server_32b_status.json"
LOG_FILE = Path("/tmp/mlx_server_32b.log")

QWEN3 = "mlx-community/Qwen2.5-32B-Instruct-4bit"
...
def clean_translation_output(raw_output, original_text):
    ...
    # ═══ PREAMBLE STRIPPING (Qwen chatty intros) ═══
    # Qwen sometimes prefixes output with meta-text like:
    #   "Certo, ecco la traduzione in italiano del testo richiesto:"
    #   "Ecco la traduzione fedele del testo:"
    #   "Sure, here is the Italian translation:"
    # These must be stripped BEFORE other cleanup to avoid false positive detection.
    preamble_patterns = [
        r'^[Cc]erto[,!.]?\s*[Ee]cco\s+la\s+traduzione[^:]*:\s*',
        r'^[Ee]cco\s+la\s+traduzione[^:]*:\s*',
        r'^[Ee]cco\s+una\s+traduzione[^:]*:\s*',
        r'^[Ee]cco\s+il\s+testo\s+tradotto[^:]*:\s*',
        r'^[Cc]erto[,!.]?\s*[Ee]cco\s+il\s+testo[^:]*:\s*',
        r'^[Cc]erto[,!.]?\s*[Qq]ui\s+di\s+seguito[^:]*:\s*',
        r'^[Ss]ure[,!.]?\s*[Hh]ere\s+is\s+the[^:]*:\s*',
        r'^[Hh]ere\s+is\s+the\s+Italian\s+translation[^:]*:\s*',
        r'^[Hh]ere\s*\'s\s+the[^:]*:\s*',
        r'^[Cc]erto[,!.]?\s*$',  # Standalone "Certo," line
        r'^[Ss]ure[,!.]?\s*$',   # Standalone "Sure," line
    ]
    for pattern in preamble_patterns:
        translation = re.sub(pattern, '', translation, flags=re.MULTILINE).strip()
    ...
    # Remove common prefixes (iterate multiple times to catch stacked prefixes)
    for _ in range(3):
        for prefix in ["Italian:", "Traduzione:", "Italiano:", "Ecco:", "Translation:",
                       "Ecco la traduzione:", "Ecco la traduzione in italiano:",
                       "Ecco la traduzione in italiano del testo richiesto:",
                       "Ecco la traduzione fedele:", "Ecco la traduzione fedele del testo:",
                       "VERSIONE MIGLIORATA:", "Versione migliorata:",
                       "Answer:", "Risposta:", "Here is", "Ecco qui:",
                       "Sure!", "Sure,", "Certo!", "Certo,", "Certo.",
    ...
    def do_POST(self):
        ...
        if self.path == '/translate':
            ...
            try:
                ...
                prompt = f"""/no_think
Sei un traduttore editoriale professionista. Traduci dall'inglese all'italiano questo brano di un saggio accademico di economia politica.

Regole:
- Scrivi italiano naturale e scorrevole, NON una traduzione letterale
- Usa costruzioni sintattiche italiane idiomatiche, evita calchi dall'inglese
- Mantieni il registro accademico ma leggibile
- Rispondi SOLO con la traduzione, senza premesse né commenti
- NON scrivere "Ecco la traduzione" o simili
...
Sei un revisore editoriale italiano esperto. Migliora questa traduzione rendendola più fluida e naturale.

Regole:
- Riformula le frasi pesanti in costruzioni più agili e italiane
- Sostituisci calchi dall'inglese con espressioni idiomatiche italiane
- Mantieni il significato e il registro accademico
- Rispondi SOLO con il testo rivisto, senza premesse né commenti

Originale inglese: {text}

Traduzione da rivedere: {translation}
